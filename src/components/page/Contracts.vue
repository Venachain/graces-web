        ...
        <el-card shadow="hover" >
            ...
            <el-table :data="contracts">
                <el-table-column :label="`${$t('i18n.scName')}`" :max-width="250" show-overflow-tooltip>
                    <template slot-scope="scope"><router-link :to = "'/contract/' + scope.row.address + '/chain/' + chainId">{{'(' + getAddress(scope.row) + ')' + scope.row.address}}</router-link></template>
                </el-table-column>
                ...
                <el-table-column :label="`Transaction`"  show-overflow-tooltip :max-width="250">
                    <template slot-scope="scope"><router-link :to = "'/tx/' + scope.row.tx_hash + '/chain/' + chainId">{{scope.row.tx_hash}}</router-link></template>
                </el-table-column>
...
export default {
    ...
    methods: {
        ...
        submitForm() {
            this.$refs.search.validate(valid => {
                if (valid) {
                    let data = {
                        chain_id: chainId,
                        contract_address: this.param.info
                    }
        ...
        submitDeploy(){
            ...
            deployContract(this.chainId, updata).then((res) => {
                //set cns
