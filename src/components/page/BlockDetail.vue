    ...
    <div v-for="item in items" v-bind:key="item.id">
        <el-row :gutter="20">
            ...
            <el-col :span="16">
                ...
                <!-- transaction count -->
                <div v-if ="item.id == 3">
                    <router-link :to = "'/tx/' + block.id + '/block/' + chainId" >
...
import { chainStats } from '@/api/chain'
export default {
     data: function() {
        return {
            block:  {
    ...
    methods:{
        getData(){
            let data = {
                ...
                height: parseInt(this.$route.params.blockNum)
            }
            getBlockList(data).then((res) => {
                if (res.data != null && res.data.page_total == 1) {
                    this.block = res.data.items[0]
                }
            })
        },
        getStatData(){
        ...
        goLeft(){
            ...
            this.$router.push('/block/' + (this.block.height - 1) + '/chain/' + this.chainId)
        },
        goRight(){
