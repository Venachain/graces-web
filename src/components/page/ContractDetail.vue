    ...
    <div v-for="item in items" v-bind:key="item.id">
        <el-row :gutter="20">
            ...
            <el-col :span="20" >
                ...
                <!-- contract creator -->
                <div v-if = "item.id == 2">
                    <router-link :to = "'/tx/' + item.value + '/participant/' + chainId" >{{item.value}}</router-link>
                    <span style="padding-left:10px;padding-right:10px">at txn</span>
                    <router-link :to = "'/tx/' + item.value2 + '/chain/' + chainId"  >{{item.value2}}</router-link>
                </div>
                ...
                <div v-if = "item.id == 3">
                    ...
                    <ul>
                      <li v-for="line in item.value"> {{line}}</li>
                    </ul>
                </div>
            </el-col>
        </el-row>
...
<script>
import { getContractByAddress } from '@/api/contract'
export default {
     data: function() {
        return {
            contract:{
    ...
    methods:{
        getData(){
            ...
            getContractByAddress(data).then((res) => {
                if (res.data != null) {
                    this.contract = res.data
                    if (this.contract.cns !== null && this.contract.cns.length > 0) {
                        this.contractName = this.contract.cns[0].name
                    }
                }
            })
        },
